...
%% \VignetteDepends{NMOF, bundesbank}
...
library("NMOF")
ff <- French("~/Downloads/French/",
             "F-F_Research_Data_Factors_daily_CSV.zip",
             frequency = "daily",
             price.series = TRUE)
plotseries(ff,
           t = as.Date(row.names(ff)),
           log.scale = TRUE,
           labels.at = "auto",
...
There is (limited) support for avoiding overlapping
labels, for case such as the following.

<<labels1,fig = TRUE, height=2.5, width=6>>=
x <- cbind(c(1, 1.03),
           c(1, 1.0301),
           c(1, 1.0302))
t <- as.Date("2019-1-1") + seq_len(NROW(x))
...
Argument \textsc{labels.min.height} defines what
fraction of the plot's height is needed for a label.

<<labels2,fig = TRUE, height=2.5, width=6>>=
plotseries(scale1(x),
           t, col = grey(c(.1,.4, .7)),
           labels = c("A", "B", "C"),
           labels.col = TRUE,
